"""
Sistema de análisis de noticias con APIs múltiples.
"""

from tracemalloc import start
from news_analyzer.config import API_KEY
from news_analyzer.exceptions import APIKeyError
from news_analyzer.api_client import fetch_news
from news_analyzer.utils import get_sources, get_articles_by_source, get_reading_time
from news_analyzer.open_ia import analyze_news_with_ia
response_data = None
try:
    response_data = fetch_news("newapi", api_key = API_KEY, query="Python")
except APIKeyError as e:
    print(f" Error : {e}")

if response_data:

    souce_set = get_sources(response_data["articles"])

    # for index,source in enumerate(souce_set,start= 1):
    #     print(f"No: {index} -- {source}")

    # articles = list(map(get_reading_time,response_data['articles']))
    # for article in articles:
    #     print(f'{article["title"]} ----- Tiempo de lectura : {article["reading_time"]}')

    # # for article in response_data["articles"]:
    # #     print(article["title"])

    # github_articles = get_articles_by_source(response_data["articles"],"github.com")
    # for github_article in github_articles:
    #     print(github_article['source']['name'], github_article['title'])
    analyze_news_with_ia(response_data['articles'],"que piensas de python")





